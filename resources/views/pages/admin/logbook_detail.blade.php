...
<div class="grid md:grid-cols-2 gap-4">
    <div class="input-group">
        ...
        <input name="name" class="input" type="text" value="{{ $internData->name }}" disabled>    
    </div> 
    <div class="input-group">
        <label class="input-label" for="">Jurusan</label>
        <input name="department" class="input" type="text" value="{{ $internData->department->name }}" disabled>    
    </div> 
    <div class="input-group">
        <label class="input-label" for="">NISN</label>
        <input name="nisn" class="input" type="text" value="{{ $internData->nisn }}" disabled>    
    </div> 
    <div class="input-group">
        <label class="input-label" for="">Industri</label>
        <input name="industry" class="input" type="text" value="" disabled>    
    </div> 
</div>
...
<div class="space-y-4">
    <x-content_group_logbook month="1">
        <x-slot name="logbookContent">
            @foreach($logbookData as $dt)
            <x-content_logbook>
                <x-slot name="period">Minggu 1 ({{ $dt->start_date}} s/d {{ $dt->end_date }})</x-slot>
                <x-slot name="content">{{ $dt->activities }}</x-slot>
            </x-content_logbook>
            @endforeach
        </x-slot>
